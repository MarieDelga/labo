...
PARAM  <- list()
PARAM$experimento <- "3F_TS_M4" # "TS9320"

PARAM$exp_input  <- "2F_FE_M4" # "FE9250"
...
PARAM$final_train  <- c( 201901, 201902, 201903, 201904, 201905, 201906, 
                         ...
                         202007, 202008,         202010, 202011, 202012, 
...
PARAM$train$validation   <- c(   201910,  202009 )
PARAM$train$testing      <- c(  201910,  202009)
...
PARAM$train$semilla_sampling  <- 432557
PARAM$train$semilla_under     <- 432557
...
#MAR grabo los datos del testing (para medir modelos)
# aqui JAMAS se hace sampling
fwrite( dataset[ foto_mes %in% PARAM$train$testing, ],
        file= "dataset_test.csv.gz",
        logical01= TRUE,
        sep= "," )
...
dataset[ foto_mes %in% PARAM$train$training & 
         ...
         ( azar_under <= PARAM$train$undersampling_mayoritaria | clase_completa %in% c( "BAJA+1", "BAJA+2", "BAJA+3" ) )
         , fold_train := 1L ]
