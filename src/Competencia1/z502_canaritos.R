...
dataset_train = dataset # dataset train lo destruyo, le agrego pajaritos 


#agrego tantos canaritos un tercio de las variables 
for( i in 1:(dim(dataset_train)[2]/3) ) dataset_train[ , paste0("canarito", i ) :=  runif( nrow(dataset_train)) ]
...
dataset_train = subset(dataset_train, select = -c(numero_de_cliente) )
...
obj_fun_md_ms <- function(x) {
  experimento_rpart(dataset_train , semillas
                    , md = x$maxdepth
                    , ms = x$minsplit
...
#bajamas2
true_positive = sum((dataset_train$clase_ternaria == "BAJA+2")   *( predict(modelo, dataset_train)[, "BAJA+2"]>= 0.025))
#529
