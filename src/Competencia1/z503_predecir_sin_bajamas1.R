...
dataset_train_canarito = dataset # dataset train lo destruyo, le agrego pajaritos 


#agrego tantos canaritos un tercio de las variables 
for( i in 1:(dim(dataset_train_canarito)[2]/3) ) dataset_train_canarito[ , paste0("canarito", i ) :=  runif( nrow(dataset_train_canarito)) ]


dataset_train_canarito<- dataset_train_canarito[foto_mes == 202101]
dataset_train_canarito<- dataset_train_canarito[clase_ternaria != "BAJA+1"]
...
for (var in variables_pesos) {
  dataset_train_canarito[, (paste(prefix, var, sep = "")) := ntile(get(var), 1000)]
  dataset_train_canarito[, (var) := NULL]}



#borramos numero de cliente
dataset_train_canarito = subset(dataset_train_canarito, select = -c(numero_de_cliente) )


#Primero  veo como quedan mis arboles
modelo_original <- rpart(
  formula= clase_ternaria ~ .,
  data= dataset_train_canarito,
  model= TRUE,
  xval= 0,
  cp= -1,
...
dataset_train_opthiperparam  <- dataset[ foto_mes==202101 ]
dataset_train_opthiperparam<- dataset_train_opthiperparam[clase_ternaria != "BAJA+1"]
...
#Time difference of 1.941638 hours
#objective: y = 21740000.000
#maxdepth=5; minsplit=284; fraccionminbucket=1.53
...
dataset_train  <- dataset[ foto_mes==202101 ]
...
sum(dataset_test$clase_ternaria == "BAJA+2")
850*78000

#bajamas2
true_positive = sum((dataset_test$clase_ternaria == "BAJA+2")   *( predict(modelo, dataset_test, type = "prob")[, "BAJA+2"]>= 0.025))
true_positive
#528
false_positive = sum((dataset_train$clase_ternaria != "BAJA+2")   *( predict(modelo, dataset_train, type = "prob")[, "BAJA+2"]>= 0.025))
false_positive
#8864

#hubo 664 baja+1
sum((dataset_test$clase_ternaria == "BAJA+1")   *( predict(modelo, dataset_test)[, "BAJA+2"]>= 0.025))
#372



test_prediccion <- predict(modelo, dataset_test, type = "prob")
ganancia(test_prediccion[, "BAJA+2"], dataset_test$clase_ternaria) 
...
prefix <- "ntile_"
for (var in variables_pesos) {
  dapply_test[, (paste(prefix, var, sep = "")) := ntile(get(var), 1000)]
  dapply_test[, (var) := NULL]}



dapply_test = dapply_test[,..mis_variables]
