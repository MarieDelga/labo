...
for (s in semillas) {
  ...
  vector_importantes <- c(vector_importantes, "clase_ternaria")
...
modelo_rpart <- function(train, test, cp =  -1, ms = 20, mb = 1, md = 10) {
  ...
  ganancia(test_prediccion[, "BAJA+2"], test$clase_ternaria) 
...
modelo <- rpart(clase_ternaria ~ ., data = dataset_train_final ,
                xval = 0,
                cp = -1,
                minsplit = 376,
                minbucket = 376*1.12,
...
true_positive = sum((dataset_test_final$clase_ternaria == "BAJA+2")   *( predict(modelo, dataset_test_final)[, "BAJA+2"]>= 0.025))
true_positive
#529
false_positive = sum((dataset_test_final$clase_ternaria != "BAJA+2")   *( predict(modelo, dataset_test_final)[, "BAJA+2"]>= 0.025))
false_positive
#8008
...
#hubo 664 baja+1
sum((dataset_test_final$clase_ternaria == "BAJA+1")   *( predict(modelo, dataset_test_final)[, "BAJA+2"]>= 0.025))
#397 cuantos de los baja+1 predijo como baja+2


test_prediccion <- predict(modelo, dataset_test_final, type = "prob")
ganancia(test_prediccion[, "BAJA+2"], dataset_test_final$clase_ternaria) 
#25.012.000
