...
#dataset set original con variables lineales



### FILTRO VARIABLES CON CANARITOS
t0 <- Sys.time()
print(t0)
mis_variables <- c()
for (s in semillas) {
  ...
  modelo_original <- rpart(
    formula= clase_binaria ~ . - clase_ternaria, #que entrene con todo menos la original
  ...
  vector_importantes <- c(vector_importantes, "clase_ternaria", "clase_binaria")
  mis_variables_s = vector_importantes [! vector_importantes %like% "canarito" ]
  mis_variables <- unique( c(mis_variables, mis_variables_s))}
...
dataset_train_opth <-copy(dataset)
dataset_train_opth  <- dataset_train_opth[ foto_mes==202101 ]

dataset_train_opth [, clase_binaria := ifelse(
  clase_ternaria == "BAJA+2",
  "evento",
  "noevento"
)]
...
modelo_rpart <- function(train, test, cp =  -1, ms = 20, mb = 1, md = 10) {
  ...
  ganancia(test_prediccion[, "evento"], test$clase_ternaria) 
...
experimento_rpart <- function(dataset_train , semillas, cp = -1, ms = 20, mb = 1, md = 10) {
  ...
  for (s in semillas) {
    set.seed(s)
    in_training <- caret::createDataPartition(dataset_train $clase_ternaria, p = 0.70,
                                              list = FALSE)
    ...
    #filtro variables luego de crear variables ranking
...
obj_fun_md_ms <- function(x) {
  experimento_rpart(dataset_train_opth  , semillas
                    , md = x$maxdepth
                    , ms = x$minsplit
...
#entreno con todos los datos el modelo ganador


set.seed(semillas[4])

dataset_train_final = copy(dataset)
dataset_train_final  <- dataset_train_final[ foto_mes==202101 ]

dataset_train_final [, clase_binaria := ifelse(
  clase_ternaria == "BAJA+2",
  "evento",
  "noevento"
)]
...
for (var in variables_pesos) {
  dataset_train_final[, (paste(prefix, var, sep = "")) := ntile(get(var), 1000)]
  dataset_train_final[, (var) := NULL]}

dataset_train_final = dataset_train_final[,..mis_variables]
...
#### elijo punto de corte

leaves_table <- function(model, dataset_train_final, target, prefix = "") {
  leaves_train_table <- data.table(
    # Devuelve en que hoja cae un caso
    leaves = rpart.predict.leaves(model, dataset_train_final, type = "where"),
    classes = dataset_train_final[, clase_ternaria],
    target = dataset_train_final[, get(target)]
  )
  leaves <- dcast(
    leaves_train_table,
...
train_bin2 <- leaves_table(modelo, dataset_train_final, "clase_binaria")
train_bin2[, gan_acum := cumsum(gan) / 0.7]
print(train_bin2)


#evaluamos modelo (nose si esta bien pero me da una idea de como comparar con publico)
dataset_test_final = copy(dataset)
dataset_test_final  <- dataset_test_final[ foto_mes==202101 ]

dataset_test_final [, clase_binaria := ifelse(
  clase_ternaria == "BAJA+2",
  "evento",
  "noevento"
)]
...
for (var in variables_pesos) {
  dataset_test_final[, (paste(prefix, var, sep = "")) := ntile(get(var), 1000)]
  dataset_test_final[, (var) := NULL]}

dataset_test_final = dataset_test_final[,..mis_variables]
...
sum(dataset_test_final$clase_ternaria == "BAJA+2")
850*78000
#66.300.000

#baja+2 cuantos de los que le tiramos incentivo eran baja+2 y cuantos no
true_positive = sum((dataset_test_final$clase_ternaria == "BAJA+2")   *( predict(modelo, dataset_test_final)[, "evento"]>= 0.025))
true_positive
#561
false_positive = sum((dataset_test_final$clase_ternaria != "BAJA+2")   *( predict(modelo, dataset_test_final)[, "evento"]>= 0.025))
false_positive
#9161


sum(dataset_test_final$clase_ternaria == "BAJA+1")
#hubo 664 baja+1
sum((dataset_test_final$clase_ternaria == "BAJA+1")   *( predict(modelo, dataset_test_final)[, "evento"]>= 0.025))
#359 cuantos de los baja+1 predijo como baja+2


test_prediccion <- predict(modelo, dataset_test_final, type = "prob")
ganancia(test_prediccion[, "evento"], dataset_test_final$clase_ternaria) 
#25436000
